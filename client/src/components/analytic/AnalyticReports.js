...
class AnalyticBasis extends Component {
  ...
  componentDidMount() {
    this.getData('foodnext');
  }

  getData = (view_project) => {
    ...
    fetch('http://localhost/xa/ga', postOption)
    // fetch('https://node.aiday.org/xa/' + type, postOption)
      ...
      .then(response => {
        ...
        this.setState({ 
            pageviewHot: response.pageviewHot,
            pageviewShare: response.pageviewShare,
            userInterest: response.userInterest,
            pageviewCate: response.pageviewCate
        });
